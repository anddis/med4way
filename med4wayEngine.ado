*! This is med4wayEngine.ado
*! v 2.0.0 - 20170327

capture program drop med4wayEngine
program define med4wayEngine, eclass
	version 10.0	

	syntax [anything] [if], [ yvar(string) ] avar(string) mvar(string) /*
...
********************************************************************************
	...
	// The following part is not optimal, it can certainly be compressed to avoid
	// repetitions (good). On the other hand, that would reuqire a lot of nested
	// ifs (hard to read -> not good). 
	// Block 1: yreg=linear, mreg=linear
	if (("`yreg'"=="linear") & ("`mreg'"=="linear")) {
		display `titley'
		regress `yvar' `avar' `mvar' `inter' `cvar' if `touse'
		matrix `Vy' = e(V)
		matrix `betay' = e(b)
		
		display `titlem'
		regress `mvar' `avar' `cvar' if `touse'
		matrix `Vm' = e(V)
		matrix `betam' = e(b)
	}	
	// End of Block 1
	...
	// Block 3: yreg=logistic, mreg=linear
		...
		display `titley'
		logit `yvar' `avar' `mvar' `inter' `cvar' if `touse'
		matrix `Vy' = e(V)
		matrix `betay' = e(b)
	
		display `titlem'
		if "`casecontrol'"=="true" {
			regressML `mvar' `avar' `cvar' if `yvar' == 0 & `touse', onlybeta(`bootstrap')
		}
		else if "`casecontrol'"=="false" {
			regressML `mvar' `avar' `cvar' if `touse', onlybeta(`bootstrap')
		}
		matrix `Vm' = e(V)
		matrix `betam' = e(b)
	}	
	...
	// Block 4: yreg=logistic, mreg=linear
		...
		display `titlem'
		if "`casecontrol'"=="true" {
			logit `mvar' `avar' `cvar' if `yvar' == 0 & `touse'
		}
		else if "`casecontrol'"=="false" {
			logit `mvar' `avar' `cvar' if `touse'
		}
	...
	// Block 5: yreg=cox, mreg=linear
		...
		display `titley'
		stcox `avar' `mvar' `inter' `cvar' if `touse'
		matrix `Vy' = e(V)
		matrix `betay' = e(b)
	...
	// Block 6: yreg=cox, mreg=logistic
		...
		display `titley'
		stcox `avar' `mvar' `inter' `cvar' if `touse', nohr
		matrix `Vy' = e(V)
		matrix `betay' = e(b)
	...
	// Block 7: yreg=aft, mreg=linear
		...
		display `titley'
		streg `avar' `mvar' `inter' `cvar' if `touse', time dist(`dist')
		matrix `Vy' = e(V)
		matrix `betay' = e(b)
	...
	// Block 10: yreg=logbinomial, mreg=logistic
		...
		display `titlem'
		logit `mvar' `avar' `cvar' if `touse', ml
		matrix `Vm' = e(V)
		matrix `betam' = e(b)
	}	
	// End of Block 10
	...
	// Block 11: yreg=poisson, mreg=linear
		...
		display `titley'
		poisson `yvar' `avar' `mvar' `inter' `cvar' if `touse', `robust'
		matrix `Vy' = e(V)
		matrix `betay' = e(b)
	...
	// End of Block 12
	 ...
	 // Block 13: yreg=negbinomial, mreg=linear
		...
		display `titley'
		nbreg `yvar' `avar' `mvar' `inter' `cvar' if `touse'
		matrix `Vy' = e(V)
...
********************************************************************************
	...
	//skip delta method calculation
	if "`deltamethod'" == "false" {
		exit
	}
...
********************************************************************************
	...
	// Step 2===================================================================
	// Estimation, delta method, bootstrap
	if "`bootstrap'" == "false" {
		foreach j of local names {
			mata: med4wayMata("`j'", "`betay'",  "`betam'", "`Vy'", "`Vm'", /*
...
program define regressML, eclass
	version 10.0
	syntax varlist(min=2 numeric) [if] [, onlybeta(string)]
...
clear mata
mata: 
void function med4wayMata(string scalar t, string scalar betay, string scalar betam, string scalar Vy, string scalar Vm, string scalar c, real scalar nc, string scalar yreg, string scalar mreg, string vector aam, string scalar boot) 
{	
	...
	p = findexternal("med4wayFormulas()") // pointer
	
	D = deriv_init()
	deriv_init_evaluator(D, &med4wayEval())
...
void med4wayEval(b, pointer(real scalar function) scalar f, t, c, nc, yreg, mreg, aam, v)
{
	v = (*f)(b, t, c, nc, yreg, mreg, aam)
}

real scalar med4wayFormulas(real vector b, string scalar t, real vector c, real scalar nc, string scalar yreg, string scalar mreg, real vector aam)
{
